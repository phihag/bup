	...
	for (var court_idx = 0;court_idx < event.courts.length;court_idx++) {
		...
		for (var team_idx = 0;team_idx < 2;team_idx++) {
			...
			for (var game_idx = 0;game_idx < max_games;game_idx++) {
				...
				uiu.el(tr, 'td', {
					style: (
						'width:1.2em;border-left:0.1vw solid #888;' +
						'padding:0 0.2em;text-align:center;'),
				}, (game_idx < nscore.length) ? nscore[game_idx][team_idx] : '');
			}
		}

		var summary_tr = uiu.el(table, 'tr', {});
		if (logo_urls) {
			uiu.el(summary_tr, 'td', {
				'style': 'color:#888;text-align:center;font-size:' + font_size,
			}, match ? match.setup.match_name : '');
		}

		var summary_td = uiu.el(summary_tr, 'td', {
			colspan: max_games + 1,
			style: 'text-align: center;color:#666;font-size:1.3vw;',
		}, team_names[0] + ' - ' + team_names[1] + '  ' + match_score[0] + '-' + match_score[1]);
	}
...
function render_streamcourt(s, container, event, colors) {
	...
	var court = event.courts.find(function(c) {
		return c.court_id == s.settings.court_id;
	}) || event.courts[0];
	...
	var table = uiu.el(container, 'table', {
		style: (
			...
			'font-size:' + (scale * 25) + 'px;color:#000;background:#ddd;'
		),
	});
			...
			if (logo_urls[team_idx]) {
				uiu.el(logo_td, 'div', {
					style: (
						'background: url("' + logo_urls[team_idx] + '") no-repeat center center;' +
						'background-size:contain;' +
						'height:' + (scale * 30) + 'px;width:' + (scale * 40) + 'px;'
					),
				});
			}
		}
	...
	var summary_tr = uiu.el(table, 'tr');
	var summary_td = uiu.el(summary_tr, 'td', {
		colspan: (logo_urls ? 2 : 1) + max_games,
	...
	match.setup.teams.forEach(function(team, team_id) {
		...
		players.map(function(player, player_id) {
			...
			uiu.el(player_container, 'div', {
				style: (
					style +
					'white-space:pre;overflow-x:hidden;width:100%;' +
					'padding-left:2vh;font-size:17vh;display:flex;align-items: center;'
				),
			}, player_name);
		});
	});
...
function render_clean(s, container, event, court, match, colors) {
	...
	match.setup.teams.forEach(function(team, team_id) {
		...
		var team_container = uiu.el(container, 'div', 'd_clean_team');
		var player_spans = player_names.map(function(pname, player_id) {
			var is_server = (!match_over) && team_serving && (server.player_id === player_id);
			var style = (
...
function option_applies(style_id, option_name) {
	var BY_STYLE = {
		...
		stream: ['team_colors', 'c0', 'c1', 'cbg'],
		streamcourt: ['court_id', 'team_colors', 'c0', 'c1', 'cbg', 'reverse_order', 'scale'],
